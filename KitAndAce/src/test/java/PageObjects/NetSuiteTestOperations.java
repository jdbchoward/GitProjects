...
public class NetSuiteTestOperations {
	...
	static Logger log = Logger.getLogger(NetSuiteTestOperations.class.getName());

	public NetSuiteTestOperations(WebDriver driver) {
		this.driver = driver;
	...
	public void doLogOnSite(String username, String psw) {
		...
		dealWithSecurityQuestion();

	}

	public boolean checkSignInStatus() {

		if (ElementExist(By.xpath("//span[contains(text(),'Howard Zhang')]")))
			return true;
		else
			return false;
	...
	private void dealWithSecurityQuestion() {
		wait.waitElementToBeDisplayed(By.xpath("//td[@class='smalltextnolink' and contains(text(),'?')]"));
		String securityQ = driver.findElement(By.xpath("//td[@class='smalltextnolink' and contains(text(),'?')]"))
				.getText();

		if (securityQ.contains("nickname"))
			driver.findElement(By.xpath("//input[@name='answer']")).sendKeys("pangzi");
		if (securityQ.contains("city"))
			driver.findElement(By.xpath("//input[@name='answer']")).sendKeys("shanghai");
		if (securityQ.contains("music"))
			driver.findElement(By.xpath("//input[@name='answer']")).sendKeys("none");

		driver.findElement(By.xpath("//input[@name='submitter']")).click();
	}
}